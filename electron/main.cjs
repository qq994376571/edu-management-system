...
function getStoredPath(username) {
  ...
  if (username) {
    const accountPaths = config.dataFolderPaths && typeof config.dataFolderPaths === 'object'
      ? config.dataFolderPaths
      : {};
    if (typeof accountPaths[username] === 'string' && accountPaths[username]) {
      return accountPaths[username];
    }

    // Older builds keyed the folder by the then-current username.  After an
    // administrator renames that account, the safe parent folder is still
    // usable because the actual data file remains account-specific.  Reuse a
    // single unambiguous stored folder instead of forcing the teacher to pick
    // it again after every launch.
    const uniqueAccountPaths = [...new Set(Object.values(accountPaths).filter(value => typeof value === 'string' && value))];
    if (uniqueAccountPaths.length === 1) return uniqueAccountPaths[0];
  }
...
function setStoredPath(folderPath, username) {
  const config = readConfig();
  // Keep the latest selected parent folder as the cross-version fallback as
  // well as the account-specific entry. Multiple accounts can safely share
  // it because their actual JSON filenames are separated by username hash.
